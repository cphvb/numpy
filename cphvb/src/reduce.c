...
{
    reduce_id = 0;
    cphvb_error err = vem_reg_func(NULL, "cphvb_reduce", &reduce_id);
               ...
               PyArrayObject *out, int axis)
{
    ...
    rinst = malloc(sizeof(cphvb_reduce_type));
    if(rinst == NULL)
    {
        PyErr_NoMemory();
        return -1;
    ...
    rinst->nout        = 1;
    rinst->nin         = 1;
    rinst->struct_size = sizeof(cphvb_reduce_type);
